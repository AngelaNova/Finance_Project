...
const FinancialSummary = () => {
  ...
  const totalExpanses =  allExpenses.reduce((totalAmounts, expense) => { return totalAmounts + expense.amount  }, 0 );
  const totalIncomes =  allIncomes.reduce((totalAmounts, income) => { return totalAmounts + income.amount  }, 0 );
  ...
  const calculateMontlyIncome = (month) => {
    let filterIncomes = allIncomes.filter((income) => {
      ...
      let total = filterIncomes.reduce((totalAmounts, income) => { return totalAmounts + income.amount  }, 0 );
  ...
  return (
    <div>
      ...
      <button onClick={() => calculateYearlyExpense(yearlyExpensesInput.current.value)}>Get total Yearly Expenses</button>
      
      <p>{totalForYearExp}</p>

      <h2>See Monthly or Yearly Incomes</h2>
      <input ref={monthylIncomesInput} type="text" required />
      <button onClick={() => calculateMontlyIncome(monthylIncomesInput.current.value)}>Get total Incomes</button>
